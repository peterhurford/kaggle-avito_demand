...
def run_ridge_on_cat(cat):
    if not is_in_cache('cat_ridges_' + cat):
        print_step(cat + ' > Subsetting')
        train_c = train[train['parent_category_name'] == cat].copy()
        test_c = test[test['parent_category_name'] == cat].copy()
        print(train_c.shape)
        print(test_c.shape)
        ...
        print_step(cat + ' > Titlecat TFIDF 1/3')
        ...
        print_step(cat + ' > Titlecat TFIDF 2/3')
        ...
        print_step(cat + ' > Titlecat TFIDF 3/3')
        tfidf_test = tfidf.transform(test_c['titlecat'])
        print(tfidf_test.shape)

        print_step(cat + ' > Titlecat TFIDF Ridge')
        results = run_cv_model(tfidf_train, tfidf_test, target, runRidge, {'alpha': 5.0}, rmse, cat + '-titlecat-ridge')
        train_c['cat_title_ridge'] = results['train']
        test_c['cat_title_ridge'] = results['test']

        print_step(cat + ' > Description TFIDF 1/3')
        train_c['desc'] = train_c['title'].fillna('') + ' ' + train_c['description'].fillna('')
        test_c['desc'] = test_c['title'].fillna('') + ' ' + test_c['description'].fillna('')
        print_step(cat + ' > Description TFIDF 2/3')
        tfidf = TfidfVectorizer(ngram_range=(1, 2),
                                max_features=100000,
                                min_df=2,
                                max_df=0.8,
        ...
        print_step(cat + ' > Description TFIDF 3/3')
        tfidf_test2 = tfidf.transform(test_c['desc'].fillna(''))
        print(tfidf_test2.shape)
        results = run_cv_model(tfidf_train2, tfidf_test2, target, runRidge, {'alpha': 5.0}, rmse, cat + '-desc-ridge')
        train_c['cat_desc_ridge'] = results['train']
        test_c['cat_desc_ridge'] = results['test']

        print_step(cat + ' > Text Char TFIDF 1/2')
        ...
        print_step(cat + ' > Text Char TFIDF 2/2')
        tfidf_test3 = tfidf.transform(test_c['desc'])
        print(tfidf_test3.shape)

        results = run_cv_model(tfidf_train3, tfidf_test3, target, runRidge, {'alpha': 5.0}, rmse, cat + '-desc-char-ridge')
        train_c['cat_desc_char_ridge'] = results['train']
        test_c['cat_desc_char_ridge'] = results['test']
        ...
        results = run_cv_model(train_c2, test_c2, target, runRidge, {'alpha': 8.0}, rmse, cat + '-text-ridge')
        train_c['cat_all_text_ridge'] = results['train']
        test_c['cat_all_text_ridge'] = results['test']

        print('~~~~~~~~~~~~~~~~~~~~~~')
        print_step(cat + ' > Dropping')
        ...
        save_in_cache('cat_ridges_' + cat, train_c, test_c)
    else:
...
pool = mp.ProcessingPool(n_nodes, maxtasksperchild=500)
dfs = pool.map(lambda c: load_cache('cat_ridges_' + c), parent_cats)
pool.close()
pool.join()
pool.terminate()
pool.restart()
...
print_step('RMSEs')
print(rmse(train_ridge['deal_probability'], train_ridge['cat_title_ridge']))     #0.2299763066787577
print(rmse(train_ridge['deal_probability'], train_ridge['cat_desc_ridge']))      #0.2285397891646121
print(rmse(train_ridge['deal_probability'], train_ridge['cat_desc_char_ridge'])) #0.22842621237885027
print(rmse(train_ridge['deal_probability'], train_ridge['cat_all_text_ridge']))  #0.2267402863402588
import pdb
pdb.set_trace()

print('~~~~~~~~~~~~~~~')
print_step('Caching...')
save_in_cache('parent_cat_ridges', train_ridge, test_ridge)
