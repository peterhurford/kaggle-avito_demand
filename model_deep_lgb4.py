...
if not is_in_cache('deep_text_feats4'):
    ...
    fselect = SelectKBest(f_regression, k=80000)
    train_ = fselect.fit_transform(train, target)
    print_step('SelectKBest 2/2')
    test_ = fselect.transform(test)
    print(train.shape)
    print(test.shape)
...
submission.to_csv('submit/submit_deep_lgb4.csv', index=False)
print_step('Done!')
