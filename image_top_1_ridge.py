...
print('~~~~~~~~~~~~~~~~')
print_step('Readjusting')
merge['image_top_1_count'] = merge.groupby('image_top_1')['image_top_1'].transform('count')
merge.loc[merge['image_top_1_count'] < 300, 'image_top_1'] = merge.loc[merge['image_top_1_count'] < 300, 'parent_category_name']
merge.drop('image_top_1_count', axis=1, inplace=True)
...
def run_ridge_on_img_bin(img_bin):
    if not is_in_cache('img_bin_ridges_' + img_bin):
        print_step(img_bin + ' > Subsetting')
        train_c = train[train['image_top_1'] == img_bin].copy()
        test_c = test[test['image_top_1'] == img_bin].copy()
        print(train_c.shape)
        print(test_c.shape)
        ...
        print_step(img_bin + ' > Titlecat TFIDF 1/3')
        ...
        print_step(img_bin + ' > Titlecat TFIDF 2/3')
        ...
        print_step(img_bin + ' > Titlecat TFIDF 3/3')
        tfidf_test = tfidf.transform(test_c['titlecat'])
        print(tfidf_test.shape)

        print_step(img_bin + ' > Titlecat TFIDF Ridge')
        results = run_cv_model(tfidf_train, tfidf_test, target, runRidge5, rmse, img_bin + '-titlecat-ridge')
        train_c['img_bin_title_ridge'] = results['train']
        test_c['img_bin_title_ridge'] = results['test']

        print_step(img_bin + ' > Description TFIDF 1/3')
        train_c['desc'] = train_c['title'].fillna('') + ' ' + train_c['description'].fillna('')
        test_c['desc'] = test_c['title'].fillna('') + ' ' + test_c['description'].fillna('')
        print_step(img_bin + ' > Description TFIDF 2/3')
        tfidf = TfidfVectorizer(ngram_range=(1, 2),
                                max_features=50000,
                                binary=True,
                                encoding='KOI8-R')
        ...
        print_step(img_bin + ' > Description TFIDF 3/3')
        tfidf_test2 = tfidf.transform(test_c['desc'].fillna(''))
        print(tfidf_test2.shape)
        results = run_cv_model(tfidf_train2, tfidf_test2, target, runRidge5, rmse, img_bin + '-desc-ridge')
        train_c['img_bin_desc_ridge'] = results['train']
        test_c['img_bin_desc_ridge'] = results['test']

        print_step(img_bin + ' > Text Char TFIDF 1/2')
        ...
        print_step(img_bin + ' > Text Char TFIDF 2/2')
        tfidf_test3 = tfidf.transform(test_c['desc'])
        print(tfidf_test3.shape)

        results = run_cv_model(tfidf_train3, tfidf_test3, target, runRidge5, rmse, img_bin + '-desc-char-ridge')
        train_c['img_bin_desc_char_ridge'] = results['train']
        test_c['img_bin_desc_char_ridge'] = results['test']
        ...
        results = run_cv_model(train_c2, test_c2, target, runRidge8, rmse, img_bin + '-text-ridge')
        train_c['img_bin_all_text_ridge'] = results['train']
        test_c['img_bin_all_text_ridge'] = results['test']

        print('~~~~~~~~~~~~~~~~~~~~~~')
        print_step(img_bin + ' > Dropping')
        ...
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print_step(img_bin + ' > Saving in Cache')
        save_in_cache('img_bin_ridges_' + img_bin, train_c, test_c)
    else:
        print(img_bin + ' already in cache! Skipping...')
    return True

img_bins = list(map(lambda s: s.replace('/', '-'), list(set(train['image_top_1'].values))))
n_cpu = mp.cpu_count()
...
res = pool.map(run_ridge_on_img_bin, img_bins)
pool.close()
pool.join()
pool.terminate()
pool.restart()
...
pool = mp.ProcessingPool(n_nodes, maxtasksperchild=500)
dfs = pool.map(lambda c: load_cache('img_bin_ridges_' + c), img_bins)
pool.close()
pool.join()
pool.terminate()
pool.restart()
...
print_step('RMSEs')
print(rmse(train_ridge['deal_probability'], train_ridge['img_bin_title_ridge']))
print(rmse(train_ridge['deal_probability'], train_ridge['img_bin_desc_ridge']))
print(rmse(train_ridge['deal_probability'], train_ridge['img_bin_desc_char_ridge']))
print(rmse(train_ridge['deal_probability'], train_ridge['img_bin_all_text_ridge']))
import pdb
pdb.set_trace()

print('~~~~~~~~~~~~~~~')
print_step('Caching...')
save_in_cache('img_bin_ridges', train_ridge, test_ridge)
